...
if __name__ == "__main__":
	...
	for i in range(0, N):
		...
		for j in range(N):
			...
			dumpStr += lhs_matvec_i_elem_j + " rnd32 = " + rhs_matvec_i_elem_j + ";\n"
			...
			dumpStr += lhs_accumulated_matvec_i + " rnd32 = " + rhs_accumulated_matvec_i + ";\n"
		...
		R[0][i] = lhs_matvec_i
		dumpStr += lhs_matvec_i + " rnd32 = " + rhs_matvec_i + ";\n"
	P = copy.deepcopy(R)
	...
	while(k < L):
		...
		for i in range(N):
			...
			dumpStr += lhs_alpha_rid + " rnd32 = " + rhs_alpha_rid + ";\n"
		...
		for i in range(0, N):
			...
			dumpStr += lhs_alpha_api + " rnd32 = " + rhs_alpha_api + ";\n"
			...
			dumpStr += lhs_next_residue + " rnd32 = " + rhs_next_residue + ";\n"
	...
	for i in range(N):
		for j in range(N):
			...
			if val == 0:
				fout.write("\t"+var+"\t fl32 : ("+str(val)+" , "+str(val)+");\n")
			else:
				fout.write("\t"+var+"\t fl32 : ("+str(val-err)+" , "+str(val+err)+");\n")
		bval = str(b[i])
		bvar = "b_{idx}".format(idx=i)
		fout.write("\t"+bvar+"\t fl32 : ("+bval+" , "+bval+");\n")
		bvar = "x_0_{idx}".format(idx=i)
		fout.write("\t"+bvar+"\t fl32 : (0.0 , 0.0);\n")
